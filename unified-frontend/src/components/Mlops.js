...
function Mlops() {
  ...
  const handleTrain = async () => {
    ...
    setTrainStatus('');
    try {
      const res = await fetch('http://localhost:8004/train', { method: 'POST' });
      const data = await res.json();
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', width: 400, maxWidth: '95vw', padding: 32, margin: '0 auto' }}>
